...
# 候选集列表,存放n只蚂蚁的路径(一只蚂蚁一个路径),一共就Antcount个路径，一共是蚂蚁数量*31个城市数量
...
while iter <  MAX_iter:
    ...
    for i in range(AntCount):
        ...
        for j in range(1, city_count):#访问剩下的city_count个城市
            ...
            temp_sum = sum((protrans))
            # 累计概率，轮盘赌选择
            cumsumprobtrans = (protrans / temp_sum).cumsum()
            ...
            if np.random.rand() >= q0:
                ...
                index = np.where(cumsumprobtrans == np.max(cumsumprobtrans))
                index = index[0]
                k = unvisit[index[0]]
            # 下一个访问城市的索引值
            candidate[i, j] = k
    ...
    for i in range(AntCount):
        for j in range(city_count - 1):
            # 当前路径比如城市23之间的信息素的增量：Q/当前蚂蚁行走的总距离的信息素
            changepheromonetable[candidate[i, j]][candidate[i][j + 1]] += Q / length.min()
        #最后一个城市和第一个城市的信息素增加量
        changepheromonetable[candidate[i, j + 1]][candidate[i, 0]] += Q / length.min()
    #信息素更新的公式：
    pheromonetable = (1 - rho) * pheromonetable + changepheromonetable
    # pheromonetable = (1 - a) * pheromonetable + a * changepheromonetable
    iter += 1
